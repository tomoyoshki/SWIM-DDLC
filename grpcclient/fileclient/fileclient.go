...
func (c Client) StartJob(ctx context.Context, job_id int, batch_size int, model_type string, model_name string) (string, error) {
	ctx, cancel := context.WithDeadline(ctx, time.Now().Add(200*time.Second))
	defer cancel()
...
func (c Client) SendJobStatusReplication(ctx context.Context, job_status map[int]*utils.JobStatus) (string, error) {
	ctx, cancel := context.WithDeadline(ctx, time.Now().Add(10*time.Second))
	defer cancel()
	buf := &bytes.Buffer{}
